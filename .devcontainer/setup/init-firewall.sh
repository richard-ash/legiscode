...
# Some tools resolve localhost to ::1 (IPv6) first, but services often bind
# to 0.0.0.0 (IPv4 only). Ensure localhost resolves to 127.0.0.1.
if grep -q "^::1.*localhost" /etc/hosts 2>/dev/null; then
    sed 's/^::1\(.*localhost\)/#::1\1/' /etc/hosts > /tmp/hosts.tmp \
...
# DNS (needed for Squid to resolve allowlisted domains).
...
# Host network (VS Code remote, Docker comms).
...
# host.docker.internal can resolve to a different subnet on Docker Desktop.
...
# Squid (proxy user) is allowed to make direct outbound connections —
# without this its own traffic would be redirected back to itself.
...
# Allow redirected traffic to reach Squid's local intercept ports.
...
# Explicit reject for clear errors on non-allowed traffic.
...
if curl --connect-timeout 5 https://foo.google.com >/dev/null 2>&1; then
    ...
    echo "PASS: foo.google.com blocked (subdomain of exact-only match)"
fi
...
if ! curl --connect-timeout 5 https://registry.npmjs.org/ >/dev/null 2>&1; then
    ...
    echo "PASS: registry.npmjs.org reachable"
fi
