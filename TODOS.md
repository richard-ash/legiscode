...
- **F-perf — Three O(n) `findIndex` scans per keypress at 12k SF Municipal rows.** `src/corpus-nav/keyboard-actions.ts:107`, `src/ui/left-panel/file-tree/use-roving-focus.ts:70`, `src/ui/left-panel/file-tree/file-tree.tsx:93`. ~36k id comparisons per arrow press. Currently under the perf budget (in-tree assertion <50ms uncached). Fix: promote `rowIndexById: ReadonlyMap<string, number>` from `useCorpusTree`, thread through `KeyboardState` and `useRovingFocus` as a REQUIRED field — no findIndex fallback path (D6). Scope honesty (D17): the typeahead's forward-scan for the next prefix match (`file-tree.tsx:106`) and `ArrowLeft`'s parent walk-back (`keyboard-actions.ts:80`) BOTH remain O(n) by design — a per-id Map can't accelerate a predicate sweep over node fields or a depth comparison. Deferred to a separate perf pass if real corpus usage shows headroom loss. Owner: `feat/file-tree-polish` (#18 in master plans-overview) — bundles with F-sticky and F-pendingFocus since all three touch the same hooks.
...
- **Context:** Surfaced by /plan-design-review Pass 7 on the locked feat/tabs plan. The plan's `## Visual Spec` and `## Accessibility & responsive` sections already establish the patterns (ARIA tablist, roving tabindex, close-button aria-label) that v1.1 polish will extend. Items 3-4 surfaced 2026-05-18 during manual QA on `feat/file-tree-polish`: opening enough sections to exceed strip width revealed both the missing affordance and the equal-shrink ellipsis behavior.
