...
- **Per-section attribution in `anchorTextDiff`.** v1 attributes every classified span to the bill's FIRST affected section; multi-section bills fall through to manual_review for secondary sections. Real attribution needs to map TextRun source-index ranges back to the structural-pass text offsets for each AMEND group, which in turn needs offset tracking through `stripChrome` + `runsToText`. Owner: a follow-up that adds a `runsTextOffsetMap(runs)` helper and threads section-offset ranges through emit-diff. Renderer already handles per-section sparse text_diff[] correctly (codex C8 lock), so this is a parser-only change.
- **Async section baseline lookup for the diff view.** v1's `lookupSectionBaseline` in `App.tsx` returns text only for the currently active section; opening a diff tab for a different section returns null and DiffView falls back to stated-source. Real fix: cache `api().corpus.read(...)` results in a (moduleId, sectionId)→text map, populate on tab open, and pass an async loader to DiffView so corpus-aligned mode works on cold open.
- **5th labelled fixture + human-validated per-span ground truth.** A15+C10 locked 5 fixtures (4 existing + 1 new); the new fixture didn't ship with this PR. The `.annotated.json` baselines committed today carry conservative count floors only — they catch regression but don't pin per-span correctness. Human-validated ground truth waits on the operator's first end-to-end dogfood of the rendered diff view against a real bill.
