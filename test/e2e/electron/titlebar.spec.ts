...
test("titlebar renders Phase-1 chrome on every platform", async () => {
  ...
  try {
    ...
    // 3. Workspace chip is the ⌘P entry point. Asserts the chip renders with
    //    a non-empty workspace label (rootLabel from corpus IPC) and the
    //    visible ⌘P kbd hint. We don't pin the exact label text — corpus
    //    rootLabel format may shift; "non-empty" + chip wiring is what this
    //    spec defends. The chip always contains a literal "/" separator
    //    between workspaceLabel and fileLabel, so the workspace label has
    //    its own data-testid hook in title-bar.tsx — checking the chip's
    //    full textContent would pass even with empty rootLabel because of
    //    the separator alone.
    const chip = titlebar.locator(".lc-workspace-chip");
    await expect(chip).toBeVisible();
    await expect(chip).toContainText("⌘P");
